                    ...
                    <tbody> 
                        @foreach ($orders as $cust)
                            <tr>
                                ...
                                <td class="capitalize whitespace-nowrap"><?= $cust->ustomer_name !== '' ? $cust->customer_name : $cust->invoice_no ?></td>
                                <td><span class="text-green-600">₱</span><span class="text-red-600">-</span><?= number_format($cust->discount_percent, 2) ?></td>
                                <td class="whitespace-nowrap"><span class="text-green-600">₱</span> <?= number_format($cust->total_discount_amount, 2) ?></td>
                                <td><span class="text-green-600">₱</span> <?= $cust->payment_amount ?></td>
                                <td><span class="text-green-600">₱</span> <?= number_format($cust->payment_change, 2) ?></td>
                                <td>
                                    <div class="font-medium bg-gradient-to-r <?= $cust->service_type == "TK" ? 'from-blue-400 to-gray-700' : 'from-orange-400 to-orange-700' ?> px-1 text-white px-2 rounded text-center whitespace-nowrap">
                                        <?= $cust->service_type == "TK" ? 'TAKE OUT' : 'DINE IN' ?>
                                    </div>
                                </td>
                                <td class="whitespace-nowrap hidden">
                                    <?= date('Y-m-d', strtotime($cust->created_at)) ?>
                                </td>
